...
function Dashboard() {
  ...
  const getAllStudent = () => {
    axios.get("http://localhost:9898/student").then((result) => {
...
const AddStudent = () => {
    axios.post("http://localhost:9898/student", studentData).then((result)=>{
        if(result.data.affectedRow>0)
        {
          getAllStudent();
          ClearBoxes();
        }
...
const UpdateRecord=()=>{
  axios.put(`http://localhost:9898/student/${studentData.id}`, studentData)
  .then((result)=>{
    if(result.data.affectedRow>0)
    {
      getAllStudent();
